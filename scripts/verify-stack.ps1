...
Invoke-Check "kubectl -n voting get pods,svc,externalsecret,secret"
...
Invoke-Check "kubectl -n voting rollout status deploy/result --timeout=2m"
Invoke-Check "kubectl -n voting rollout status deploy/worker --timeout=2m"
